...
public class LockRecipeSubCMD extends SubCommand {
    ...
    protected void handleCommand(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Text.color("&cUsage: /wi lock <recipe> [player]"));
            return;
        }
        ...
        if (item.hasRecipe()) {
            getPlugin().getLanguageManager().sendMessage(sender, Message.NO_RECIPE_MESSAGE.getPath(), r -> r.replace("{0}", args[0]));
            return;
        }
