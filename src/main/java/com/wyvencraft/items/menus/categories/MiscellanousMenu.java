...
public class MiscellanousMenu extends ItemMenu {
    public MiscellanousMenu(List<Item> itemList) {
        super(itemList);
    }

    enum Type {
        ORBS(ItemStackBuilder.from(Material.PLAYER_HEAD).setName("&eOrbs", true), ItemType.ORB, SlotPos.of(1, 0)),
        POTIONS(ItemStackBuilder.from(Material.BOW).setName("&ePotions", true), ItemType.POTION, SlotPos.of(2, 0)),
        ITEMS(ItemStackBuilder.from(Material.NETHERITE_AXE).setName("&eItems", true), ItemType.ITEM, SlotPos.of(3, 0)),
        BLOCKS(ItemStackBuilder.from(Material.SHIELD).setName("&eBlocks (coming soon)", true), ItemType.BLOCK, SlotPos.of(4, 0));
